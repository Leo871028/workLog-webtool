...
type LogRow={log_date:string;completed:string|null;ongoing:string|null;blockers:string|null;next_plan:string|null};
...
function logsToCsv(data:LogRow[]){const rows=[["Date","Completed Today","Ongoing / In Progress","Blockers / Issues","Next Plan"],...data.map(r=>[r.log_date,r.completed||"",r.ongoing||"",r.blockers||"",r.next_plan||""])];return "\ufeff"+rows.map(row=>row.map(csvCell).join(",")).join("\r\n")}
function logsToExcel(data:LogRow[]){const rows=data.map(r=>({Date:r.log_date,"Completed Today":r.completed||"","Ongoing / In Progress":r.ongoing||"","Blockers / Issues":r.blockers||"","Next Plan":r.next_plan||""})),ws=XLSX.utils.json_to_sheet(rows);ws["!cols"]=[{wch:14},{wch:45},{wch:45},{wch:45},{wch:45}];ws["!autofilter"]={ref:`A1:E${data.length+1}`};const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,"Daily Work Log");return new Uint8Array(XLSX.write(wb,{bookType:"xlsx",type:"array"}) as ArrayBuffer)}
Deno.serve(async req=>{
 ...
 const{data,error}=await supabase.from("daily_logs").select("log_date,completed,ongoing,blockers,next_plan").eq("user_id",exportUserId).gte("log_date",start).lte("log_date",end).order("log_date",{ascending:true});
